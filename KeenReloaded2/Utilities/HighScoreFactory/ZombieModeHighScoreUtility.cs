...
namespace KeenReloaded2.Utilities.HighScoreFactory
{
    public class ZombieModeHighScoreUtility : IHighScoreUtility
    {
        ...
        {
            var items = FileIOUtility.ReadHighScoresByGameModeAndLevel(MainMenuConstants.OPTION_LABEL_ZOMBIE_MODE, mapName);
            List<IHighScore> highScores = items
                .Select(i => (IHighScore)new ZombieModeHighScore(i.Item1, mapName, long.Parse(i.Item2)))
                .ToList();
            return highScores;
        }
        ...
        public bool WriteHighScores(List<IHighScore> highScores, string mapName)
        {
            try
            {
                var data = highScores
                    ...
                    .ToList();
                FileIOUtility.WriteHighScoresByGameMode(MainMenuConstants.OPTION_LABEL_ZOMBIE_MODE, mapName, data);
                return true;
            }
            catch (Exception ex)
            {
